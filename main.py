...
if __name__ == '__main__':
    ...
    # args = parser.parse_args(['ppc','--DP','True','--train','True','--use','True'])  # 数据初始化
    ...
    # args = parser.parse_args(Linux + base) # 训练模型(正常训练的情况下，解开这一行的注释，然后注释下一行，记得GpuNum和--nproc-per-node后的参数要改得和真实的gpu数量一致)
    args = parser.parse_args()

    args.func(args)
# screen -dmS test bash -c "./venv/bin/torchrun --nproc-per-node 8 ./Transfomer/main.py train --DataPath /root/autodl-tmp/Data/ --GpuNum 8" # 训练指令
# ./venv/bin/python ./Transfomer/main.py test --Greedy True --Beam False --num 100000 --GpuNum 1 --batch_size 5# 测试 RTX 2080Ti * 4(服务器测试) / RTX 5070Ti * 1(本地测试)